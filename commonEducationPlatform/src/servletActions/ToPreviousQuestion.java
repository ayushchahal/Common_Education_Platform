...
public class ToPreviousQuestion extends HttpServlet{
		...
		try {
			...
			else 
			{
				...
				if(numberOfQuestionsSaved >= questionNumber)
				{
					...
					{
						nextQuestionID = qIDs[questionNumber-2];
					}
					System.out.println(nextQuestionID);
					if(tc.isQuestionUpdated(""+questionID, questionText, questionType, pmarks, nmarks, options[0], options[1], options[2], options[3], options[4], options[5], answer, timeLimit))
					{
						System.out.println("Doing update operation");
						TestUpdater tu=new TestUpdater();
						tu.updateQuestion(""+questionID, questionText, questionType, pmarks, nmarks, options[0], options[1], options[2], options[3], options[4], options[5], answer, timeLimit);
					...
					{
						System.out.println("Display previous question without doing any update operation.");
						request.setAttribute("questionID", nextQuestionID);
						RequestDispatcher rd=request.getRequestDispatcher("/CreateQuestions");
						rd.forward(request,response);
					}
				...
				else
				{
					...
					tc.insertQuestionDetails(questionText, questionType, pmarks, nmarks, options[0], options[1], options[2], options[3], options[4], options[5], testID, answer, timeLimit);					
					RequestDispatcher rd=request.getRequestDispatcher("/CreateQuestions");
					rd.forward(request,response);
				}
