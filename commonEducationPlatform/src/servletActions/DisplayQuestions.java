		...
		try {
			...
			else 
			{
				...
				*/
				...
				int totalTimeInSeconds = tc.getTestTime(testID)*60;
				int hours = 0;
				...
				String html = "<html><head><meta charset=\"utf-8\">\r\n" + 
						...
						"    <link rel=\"stylesheet\" href=\"\\stylesheets\\displayQuestions.css\"><script src=\"\\javascript\\timer.js\"></script></head>";
				...
				String previousButton = "<input class=\"btn btn-primary float-left\" type=\"submit\" name=\"save\" value=\"To Previous Question\" style=\"margin-left:2%\">";
				//String nextButton="<input class=\"rhs\" type=\"submit\" name=\"save\" value=\"To Next Question\">";
				String nextButton="<input class=\"btn btn-primary float-right\" type=\"submit\" name=\"save\" value=\"To Next Question\" style=\"margin-right:2%\">";
				//String submit="<input class=\"rhs\" type=\"submit\" name=\"save\" value=\"Submit test\">";
				String submit="<input value=\"Submit test\" name=\"save\" type=\"submit\" class=\"btn btn-success\" style=\"margin-left:45%;\">";
				...
				*/
				...
				else if(questionType.equals("3"))
				{
					for (int i=0;i<n;i++)
					{
						rb1 = rb1 +inputGroup+ "<input type=\"checkbox\" name=\"options\" value=\""+(i+1)+"\">"+options[i]+"</h7>\r\n" + 
								"</div>\r\n" + 
								"    </label>";
					}
				}
				...
				if(questionNumber == 1)
					out.print();
				else if(questionNumber == numberOfQuestions)
					out.print();
				else
					out.print();
